...
const props = defineProps({ houses: Object, villages: Array, bariRepresentatives: Array, filters: Object });
const { t } = useI18n();
...
const filterForm = reactive({
    ...
    has_representative: props.filters?.has_representative || '',
});

const applyFilters = () => {
    const params = {};
    Object.entries(filterForm).forEach(([k, v]) => { if (v) params[k] = v; });
    router.get(route('admin.houses.index'), params, { preserveState: true, replace: true });
};
...
<template>
    ...
    <AppLayout>
        ...
        <div class="bg-white rounded-xl border p-4 mb-6">
            <h2 class="text-sm font-semibold text-slate-700 mb-3">{{ t('filter_houses') }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                <input v-model="filterForm.search" @keyup.enter="applyFilters" :placeholder="t('search_house_placeholder')" class="rounded-lg border-slate-300 text-sm" />
                <select v-model="filterForm.village_id" class="rounded-lg border-slate-300 text-sm">
                    <option value="">{{ t('all_villages') }}</option>
                    <option v-for="v in villages" :key="v.id" :value="v.id">Ward {{ v.ward_number }} — {{ v.name_bn }}</option>
                </select>
                <select v-model="filterForm.has_representative" class="rounded-lg border-slate-300 text-sm">
                    <option value="">{{ t('all_representatives') }}</option>
                    <option value="yes">{{ t('has_representative') }}</option>
                    <option value="no">{{ t('no_representative_assigned') }}</option>
                </select>
            </div>
        ...
        <div v-if="houses.links?.length > 3" class="flex gap-1 mt-4 flex-wrap">
            <Link v-for="link in houses.links" :key="link.label" :href="link.url" v-html="link.label"
